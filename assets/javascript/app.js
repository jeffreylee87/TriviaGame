...
//won't change gif's, messed it up too many times
...
var qA = [{
    ...
    options: ["Gene Vegan High School", "Tom Landry High School", "Cyprss High School", "Harry Herpson High School"],
    answer: 3,
}, {
...
function loadQuestions (){
    ...
    for(var i in qA){
        storeIndex = i;
        $(".box1").html(`<div>${qA[i].question}</div>`);
        $(".box2").html(`<div class="answer">${qA[i].options[0]}</div>
        <div class="answer">${qA[i].options[1]}</div><div class="answer">${qA[i].options[2]}</div><div class="answer">${qA[i].options[3]}</div>`);
